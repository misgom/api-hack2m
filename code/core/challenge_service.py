...
async def log_attempt(attempt: ChallengeAttempt) -> None:
    """Log a challenge attempt."""
    logger.info(
        f"Challenge attempt logged: {attempt.challenge_id} by user {attempt.user_uuid}"
    )

async def challenge_code_agent(request: AskRequest) -> str:
    """Challenge code agent."""
    challenge = get_challenge("excessive-agency")
    ...
    if match:
        ...
        result_output = await execute_code(code, challenge.flag)
...
async def execute_code(code: str, flag: str) -> str:
...
async def challenge_file_agent(request: AskRequest) -> str:
    """Challenge file agent."""
    challenge = await get_challenge("indirect-prompt-injection")
    llm_handler = LLMHandler.get_instance()
